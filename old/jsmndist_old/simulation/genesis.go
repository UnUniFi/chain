...
// RandomizedGenState generates a random GenesisState for jsmndist module
func RandomizedGenState(simState *module.SimulationState) {
	...
	jsmndistGenesis := types.NewGenesisState(params, types.DefaultPreviousBlockTime)
	if err := jsmndistGenesis.Validate(); err != nil {
		panic(err)
	}

	fmt.Printf("Selected randomly generated %s parameters:\n%s\n", types.ModuleName, codec.MustMarshalJSONIndent(simState.Cdc, jsmndistGenesis))
	simState.GenState[types.ModuleName] = simState.Cdc.MustMarshalJSON(jsmndistGenesis)
}
