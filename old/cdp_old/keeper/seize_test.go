...
func (suite *SeizeTestSuite) TestSeizeCollateral() {
	...
	suite.Equal(p.Int64(), acc.GetCoins().AmountOf("jpyx").Int64())
...
func (suite *SeizeTestSuite) TestKeeperLiquidation() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			_, err := pk.SetPrice(suite.ctx, sdk.AccAddress{}, "btc:jpy", tc.args.initialPrice, suite.ctx.BlockTime().Add(time.Hour*24))
			suite.Require().NoError(err)
			err = pk.SetCurrentPrices(suite.ctx, "btc:jpy")
			suite.Require().NoError(err)
			...
			// update pricefeed
			_, err = pk.SetPrice(suite.ctx, sdk.AccAddress{}, "btc:jpy", tc.args.finalPrice, suite.ctx.BlockTime().Add(time.Hour*24))
			suite.Require().NoError(err)
			err = pk.SetCurrentPrices(suite.ctx, "btc:jpy")
			suite.Require().NoError(err)
	...
	testCases := []test{
		{
			...
			args{
				...
				sdk.Coins{c("jpyx", 1000000000), c("jpyx", 500000000)},
				auction.Auctions{
					auction.CollateralAuction{
						BaseAuction: auction.BaseAuction{
							ID:              1,
		...
		},
		{
			...
			args{
				...
				d("10000.00"),
				sdk.Coins{c("btc", 10000000), c("btc", 10000000)},
				sdk.Coins{c("jpyx", 500000000), c("jpyx", 500000000)},
				auction.Auctions{},
			},
			errArgs{
				false,
