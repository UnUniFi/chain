...
func (suite *ParamTestSuite) TestParamValidation() {
	type args struct {
		jpyxMintingRewardPeriods   types.RewardPeriods
	...
	testCases := []test{
		{
			"default",
			args{
				jpyxMintingRewardPeriods:   types.DefaultRewardPeriods,
		...
		},
		{
			"valid",
			args{
				jpyxMintingRewardPeriods: types.RewardPeriods{types.NewRewardPeriod(
					true, "bnb-a", time.Date(2020, 10, 15, 14, 0, 0, 0, time.UTC), time.Date(2024, 10, 15, 14, 0, 0, 0, time.UTC),
					sdk.NewCoin(types.JPYXMintingRewardDenom, sdk.NewInt(122354)))},
				multipliers: types.Multipliers{
					types.NewMultiplier(
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			params := types.NewParams(tc.args.jpyxMintingRewardPeriods, tc.args.hardSupplyRewardPeriods,
