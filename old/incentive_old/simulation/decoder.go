...
func DecodeStore(cdc *codec.Codec, kvA, kvB kv.Pair) string {
	...
	case bytes.Equal(kvA.Key[:1], types.JPYXMintingClaimKeyPrefix):
		var claimA, claimB types.JPYXMintingClaim
		cdc.MustUnmarshalBinaryBare(kvA.Value, &claimA)
		cdc.MustUnmarshalBinaryBare(kvB.Value, &claimB)
		return fmt.Sprintf("%v\n%v", claimA, claimB)

	case bytes.Equal(kvA.Key[:1], types.PreviousJPYXMintingRewardAccrualTimeKeyPrefix):
		var timeA, timeB time.Time
		cdc.MustUnmarshalBinaryBare(kvA.Value, &timeA)
		cdc.MustUnmarshalBinaryBare(kvB.Value, &timeB)
		return fmt.Sprintf("%s\n%s", timeA, timeB)

	case bytes.Equal(kvA.Key[:1], types.JPYXMintingRewardFactorKeyPrefix):
