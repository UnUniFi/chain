...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/UnUniFi/chain/simapp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
...
func fullAppSimulation(tb testing.TB, is_testing bool) {
	...
	_, simParams, simErr := simulation.SimulateFromSeed(
		...
		os.Stdout,
		ununifi.BaseApp,
		AppStateFn(ununifi.AppCodec(), ununifi.SimulationManager()),
		simulation2.RandomAccounts,                                          // Replace with own random account function if using keys other than secp256k1
		sdkSimapp.SimulationOperations(ununifi, ununifi.AppCodec(), config), // Run all registered operations
		ununifi.ModuleAccountAddrs(),
		config,
		ununifi.AppCodec(),
	)

	// export state and simParams before the simulation error is checked
	if err = sdkSimapp.CheckExportSimulation(ununifi, config, simParams); err != nil {
		tb.Fatal(err)
	}
