...
import (
	...
	"github.com/UnUniFi/chain/tests/e2e/util"
...
func (n *internalNode) buildCreateValidatorMsg(amount sdk.Coin) (sdk.Msg, error) {
	...
	return stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(GetAddress(*n.keyInfo)),
		valPubKey,
		amount,
		description,
		commissionRates,
...
func (n *internalNode) export() *Node {
	return &Node{
		...
		Mnemonic:      n.mnemonic,
		PublicAddress: GetAddress(*n.keyInfo).String(),
		PublicKey:     GetPubKey(*n.keyInfo).Address().String(),
		PeerId:        n.peerId,
...
func (n *internalNode) init() error {
	...
	appState, err := json.MarshalIndent(ununifiApp.ModuleBasics.DefaultGenesis(util.Cdc), "", " ")
	if err != nil {
...
func GetPubKey(keyInfo keyring.Record) cryptotypes.PubKey {
	pubKey, err := keyInfo.GetPubKey()
	if err != nil {
		panic("error GetPubKey")
	}
	return pubKey
}

func GetAddress(keyInfo keyring.Record) types.AccAddress {
	GetPubKey(keyInfo)
	pubKey := GetPubKey(keyInfo)
