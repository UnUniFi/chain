...
import (
	...
	"github.com/UnUniFi/chain/tests/e2e/configurer/chain"
	"github.com/UnUniFi/chain/tests/e2e/containers"
	"github.com/UnUniFi/chain/tests/e2e/initialization"
...
func (bc *baseConfigurer) runValidators(chainConfig *chain.Config) error {
	...
	// done := make(chan struct{}, len(chainConfig.NodeConfigs))
	...
	for _, node := range chainConfig.NodeConfigs {
		...
		fmt.Printf("outside NodeConfigs %s\n", node.Name)
		go func(node chain.NodeConfig) {
			// defer close(done)
			defer wg.Done()

			// fmt.Printf("inside NodeConfigs %s\n", node.Name)
			if err := node.Run(); err != nil {
				fmt.Printf("inside error %s: %s\n", node.Name, err)
			}
		}(*node)
	...
	bc.t.Logf("done lunch container")
	bc.t.Logf("done lunch container")
	bc.t.Logf("done lunch container")
