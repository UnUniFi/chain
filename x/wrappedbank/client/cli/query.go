...
`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			res, err := queryClient.DenomMetadata(cmd.Context(), &banktypes.QueryDenomMetadataRequest{Denom: denom})
			if err != nil {
				return err
			}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if denom == "" {
				res, err := queryClient.TotalSupply(ctx, &banktypes.QueryTotalSupplyRequest{Pagination: pageReq})
				if err != nil {
					return err
				}
			...
			res, err := queryClient.SupplyOf(ctx, &banktypes.QuerySupplyOfRequest{Denom: denom})
			if err != nil {
				return err
			}
