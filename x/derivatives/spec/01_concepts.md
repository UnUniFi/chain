...
Because pool asset will take the counter part of the arbitral trade by a user, users can trade with high levarage and no slippage to the price from oracle with low fee.
...
Fees earned on the platform are directly added to the pool.　Therefore, DLP holders can benefit from them as a reward.   
...
There's no fee for borrowing in the current specification.
