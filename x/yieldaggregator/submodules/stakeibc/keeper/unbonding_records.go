...
// - error
func (k Keeper) GetHostZoneUnbondingMsgs(ctx sdk.Context, hostZone types.HostZone) ([]sdk.Msg, sdk.Int, []byte, error) {
	...
	if delegationAccount == nil || delegationAccount.GetAddress() == "" {
		...
		k.Logger(ctx).Error(errMsg)
		return nil, sdk.ZeroInt(), nil, sdkerrors.Wrap(types.ErrHostZoneICAAccountNotFound, errMsg)
	}
	validators := hostZone.GetValidators()
	if totalAmtToUnbond.IsZero() {
		return nil, sdk.ZeroInt(), nil, nil
	}
	// we distribute the unbonding based on our target weights
	newUnbondingToValidator, err := k.GetTargetValAmtsForHostZone(ctx, hostZone, totalAmtToUnbond.Uint64())
	if err != nil {
		errMsg := fmt.Sprintf("Error getting target val amts for host zone %s %d: %s", hostZone.ChainId, totalAmtToUnbond, err)
		k.Logger(ctx).Error(errMsg)
		return nil, sdk.ZeroInt(), nil, sdkerrors.Wrap(types.ErrNoValidatorAmts, errMsg)
	}
	...
	for _, validator := range validators {
		...
		if err != nil {
			...
			k.Logger(ctx).Error(errMsg)
			return nil, sdk.ZeroInt(), nil, sdkerrors.Wrap(types.ErrIntCast, errMsg)
		}
	...
	if overflowAmt > 0 { // what?? we still can't cover the overflow? something is very wrong
		...
		k.Logger(ctx).Error(errMsg)
		return nil, sdk.ZeroInt(), nil, sdkerrors.Wrap(sdkerrors.ErrNotFound, errMsg)
	}
	...
	if err != nil {
		...
		return nil, sdk.ZeroInt(), nil, sdkerrors.Wrap(sdkerrors.ErrNotFound, err.Error())
	}
