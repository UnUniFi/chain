...
func (k Keeper) RedeemStake(ctx sdk.Context, sender sdk.AccAddress, amount sdk.Coin, receiver string) error {
	...
	if !found {
		return sdkerrors.Wrapf(types.ErrEpochNotFound, "epoch tracker not found: %s", "day")
	}
	...
	if err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidCoins, "could not parse inCoin: %s. err: %s", coinString, err.Error())
	}
	// safety checks on the coin
	// 	- Redemption amount must be positive
	if !nativeAmount.IsPositive() {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidCoins, "amount must be greater than 0. found: %d", amount.Amount)
	}
	...
	if !found {
		...
		return sdkerrors.Wrapf(recordstypes.ErrEpochUnbondingRecordNotFound, "latest epoch unbonding record not found")
	}
	...
	redeemCoin := sdk.NewCoins(sdk.NewCoin(coinDenom, amount.Amount))
	...
	if err != nil {
		...
		return sdkerrors.Wrapf(types.ErrInsufficientFunds, "couldn't send %d derivative %s tokens to module account. err: %s", amount.Amount, hostZone.HostDenom, err.Error())
	}
	...
	k.Logger(ctx).Info(fmt.Sprintf("executed redeem stake: %s", amount.String()))
	return nil
}
