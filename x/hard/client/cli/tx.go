...
func getCmdBorrow(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Example: fmt.Sprintf(
			`%s tx %s borrow 1000000000ujsmn --from <key>`, version.ClientName, types.ModuleName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			inBuf := bufio.NewReader(cmd.InOrStdin())
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
jpyxcli tx hard repay 1000000000ujsmn --from <key>
jpyxcli tx hard repay 1000000000ujsmn,25000000000bnb --from <key>
jpyxcli tx hard repay 1000000000ujsmn,25000000000bnb --owner <owner-address> --from <key>
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
			inBuf := bufio.NewReader(cmd.InOrStdin())
			cliCtx := context.NewCLIContext().WithCodec(cdc)
