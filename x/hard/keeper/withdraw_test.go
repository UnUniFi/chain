...
func (suite *KeeperTestSuite) TestLtvWithdraw() {
	...
	testCases := []liqTest{
		{
			...
			args{
				...
				initialBorrowerCoins: sdk.NewCoins(sdk.NewCoin("ujsmn", sdk.NewInt(100*KAVA_CF)), sdk.NewCoin("jpyx", sdk.NewInt(100*KAVA_CF))),
				depositCoins:         sdk.NewCoins(sdk.NewCoin("ujsmn", sdk.NewInt(100*KAVA_CF))), // 100 * 2 = $200
				borrowCoins:          sdk.NewCoins(sdk.NewCoin("ujsmn", sdk.NewInt(80*KAVA_CF))),  // 80 * 2 = $160
				repayCoins:           sdk.NewCoins(sdk.NewCoin("ujsmn", sdk.NewInt(60*KAVA_CF))),  // 60 * 2 = $120
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err = suite.keeper.Withdraw(suite.ctx, tc.args.borrower, sdk.NewCoins(sdk.NewCoin("ujsmn", sdk.OneInt())))
			suite.Require().Error(err)
			suite.Require().True(strings.Contains(err.Error(), tc.errArgs.contains))
			...
			withdrawCoins := sdk.NewCoins(sdk.NewCoin("ujsmn", tc.args.depositCoins[0].Amount.Quo(sdk.NewInt(10))))
