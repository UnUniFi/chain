...
func (k Keeper) ValidateBorrow(ctx sdk.Context, borrower sdk.AccAddress, amount sdk.Coins) error {
	...
	// Get the proposed borrow JPY value
	proprosedBorrowJPYValue := sdk.ZeroDec()
	for _, coin := range amount {
		...
		// Calculate this coin's JPY value and add it borrow's total JPY value
		...
		coinJPYValue := sdk.NewDecFromInt(coin.Amount).Quo(sdk.NewDecFromInt(moneyMarket.ConversionFactor)).Mul(assetPriceInfo.Price)
		...
		proprosedBorrowJPYValue = proprosedBorrowJPYValue.Add(coinJPYValue)
	}

	// Get the total borrowable JPY amount at user's existing deposits
	...
	for _, coin := range deposit.Amount {
		...
		depositJPYValue := sdk.NewDecFromInt(coin.Amount).Quo(sdk.NewDecFromInt(moneyMarket.ConversionFactor)).Mul(assetPriceInfo.Price)
		borrowableAmountForDeposit := depositJPYValue.Mul(moneyMarket.BorrowLimit.LoanToValue)
		totalBorrowableAmount = totalBorrowableAmount.Add(borrowableAmountForDeposit)
	}

	// Get the total JPY value of user's existing borrows
	existingBorrowJPYValue := sdk.ZeroDec()
	...
	if found {
		for _, coin := range existingBorrow.Amount {
			...
			// Calculate this borrow coin's JPY value and add it to the total previous borrowed JPY value
			assetPriceInfo, err := k.pricefeedKeeper.GetCurrentPrice(ctx, moneyMarket.SpotMarketID)
			if err != nil {
				return sdkerrors.Wrapf(types.ErrPriceNotFound, "no price found for market %s", moneyMarket.SpotMarketID)
			}
			coinJPYValue := sdk.NewDecFromInt(coin.Amount).Quo(sdk.NewDecFromInt(moneyMarket.ConversionFactor)).Mul(assetPriceInfo.Price)
			existingBorrowJPYValue = existingBorrowJPYValue.Add(coinJPYValue)
		}
	}

	// Borrow's updated total JPY value must be greater than the minimum global JPY borrow limit
	totalBorrowJPYValue := proprosedBorrowJPYValue.Add(existingBorrowJPYValue)
	if totalBorrowJPYValue.LT(k.GetMinimumBorrowJPYValue(ctx)) {
		return sdkerrors.Wrapf(types.ErrBelowMinimumBorrowValue, "the proposed borrow's JPY value $%s is below the minimum borrow limit $%s", totalBorrowJPYValue, k.GetMinimumBorrowJPYValue(ctx))
	}

	// Validate that the proposed borrow's JPY value is within user's borrowable limit
	if proprosedBorrowJPYValue.GT(totalBorrowableAmount.Sub(existingBorrowJPYValue)) {
