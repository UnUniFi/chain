...
func (k Keeper) ValidateRepay(ctx sdk.Context, sender, owner sdk.AccAddress, coins sdk.Coins) error {
	...
	if found {
		for _, coin := range existingBorrow.Amount {
			...
			coinJPYValue := sdk.NewDecFromInt(coin.Amount).Quo(sdk.NewDecFromInt(moneyMarket.ConversionFactor)).Mul(assetPrice)
	...
	repayTotalJPYValue := sdk.ZeroDec()
	for _, repayCoin := range coins {
		...
		// Calculate this coin's JPY value and add it to the repay's total JPY value
		...
		coinJPYValue := sdk.NewDecFromInt(repayCoin.Amount).Quo(sdk.NewDecFromInt(moneyMarket.ConversionFactor)).Mul(assetPrice)
		repayTotalJPYValue = repayTotalJPYValue.Add(coinJPYValue)
	}

	// If the proposed repayment would results in a borrowed JPY value below the minimum borrow JPY value, reject it.
	// User can overpay their loan to close it out, but underpaying by such a margin that the JPY value is in an
	// invalid range is not allowed
	// Unless the user is fully repaying their loan
	proposedBorrowNewJPYValue := existingBorrowJPYValue.Sub(repayTotalJPYValue)
	isFullRepayment := coins.IsEqual(existingBorrow.Amount)
	if proposedBorrowNewJPYValue.LT(k.GetMinimumBorrowJPYValue(ctx)) && !isFullRepayment {
		return sdkerrors.Wrapf(types.ErrBelowMinimumBorrowValue, "the proposed borrow's JPY value $%s is below the minimum borrow limit $%s", proposedBorrowNewJPYValue, k.GetMinimumBorrowJPYValue(ctx))
	}
