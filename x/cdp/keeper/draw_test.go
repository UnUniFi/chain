...
func (suite *DrawTestSuite) SetupTest() {
	...
	authGS := app.NewAuthGenState(
		addrs,
		[]sdk.Coins{
			cs(c("xrp", 500000000), c("btc", 500000000), c("jpyx", 10000000000)),
			cs(c("xrp", 200000000)),
	...
	suite.ctx = ctx
	suite.addrs = addrs
	err := suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 400000000), c("jpyx", 10000000))
	suite.NoError(err)
}

func (suite *DrawTestSuite) TestAddRepayPrincipal() {

	err := suite.keeper.AddPrincipal(suite.ctx, suite.addrs[0], "xrp", c("jpyx", 10000000))
	suite.NoError(err)

	t, found := suite.keeper.GetCDP(suite.ctx, "xrp", uint64(1))
	suite.True(found)
	suite.Equal(c("jpyx", 20000000), t.Principal)
	...
	suite.Equal(ts[0], t)
	tp := suite.keeper.GetTotalPrincipal(suite.ctx, "xrp", "jpyx")
	...
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[0], "xrp", c("sjpy", 10000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidDebtRequest))

	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[1], "xrp", c("jpyx", 10000000))
	suite.Require().True(errors.Is(err, types.ErrCdpNotFound))
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[0], "xrp", c("xjpy", 10000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidDebtRequest))
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[0], "xrp", c("jpyx", 311000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidCollateralRatio))

	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp", c("jpyx", 10000000))
	suite.NoError(err)

	t, found = suite.keeper.GetCDP(suite.ctx, "xrp", uint64(1))
	suite.True(found)
	suite.Equal(c("jpyx", 10000000), t.Principal)
	...
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp", c("xjpy", 10000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidPayment))
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[1], "xrp", c("xjpy", 10000000))
	suite.Require().True(errors.Is(err, types.ErrCdpNotFound))

	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp", c("jpyx", 9000000))
...
func (suite *DrawTestSuite) TestRepayPrincipalOverpay() {
	err := suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp", c("jpyx", 20000000))
	suite.NoError(err)
	ak := suite.app.GetAccountKeeper()
	acc := ak.GetAccount(suite.ctx, suite.addrs[0])
	suite.Equal(i(10000000000), (acc.GetCoins().AmountOf("jpyx")))
	_, found := suite.keeper.GetCDP(suite.ctx, "xrp", 1)
	suite.False(found)
}

func (suite *DrawTestSuite) TestAddRepayPrincipalFees() {
	err := suite.keeper.AddCdp(suite.ctx, suite.addrs[2], c("xrp", 1000000000000), c("jpyx", 100000000000))
	suite.NoError(err)
	...
	suite.NoError(err)
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[2], "xrp", c("jpyx", 10000000))
	suite.NoError(err)
	t, _ := suite.keeper.GetCDP(suite.ctx, "xrp", uint64(2))
	suite.Equal(c("jpyx", 92827), t.AccumulatedFees)
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[2], "xrp", c("jpyx", 100))
	suite.NoError(err)
	t, _ = suite.keeper.GetCDP(suite.ctx, "xrp", uint64(2))
	suite.Equal(c("jpyx", 92727), t.AccumulatedFees)
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[2], "xrp", c("jpyx", 100010092727))
	suite.NoError(err)
	_, f := suite.keeper.GetCDP(suite.ctx, "xrp", uint64(2))
	suite.False(f)

	err = suite.keeper.AddCdp(suite.ctx, suite.addrs[2], c("xrp", 1000000000000), c("jpyx", 100000000))
	suite.NoError(err)

	suite.ctx = suite.ctx.WithBlockTime(suite.ctx.BlockTime().Add(time.Second * 31536000)) // move forward one year in time
	err = suite.keeper.UpdateFeesForAllCdps(suite.ctx, "xrp")
	suite.NoError(err)
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[2], "xrp", c("jpyx", 100000000))
	suite.NoError(err)
	t, _ = suite.keeper.GetCDP(suite.ctx, "xrp", uint64(3))
	suite.Equal(c("jpyx", 5000000), t.AccumulatedFees)
}
...
func (suite *DrawTestSuite) TestPricefeedFailure() {
	...
	err := suite.keeper.AddPrincipal(ctx, suite.addrs[0], "xrp", c("jpyx", 10000000))
	suite.Error(err)
	err = suite.keeper.RepayPrincipal(ctx, suite.addrs[0], "xrp", c("jpyx", 10000000))
	suite.NoError(err)
}
...
func (suite *DrawTestSuite) TestModuleAccountFailure() {
	suite.Panics(func() {
		...
		suite.keeper.RepayPrincipal(ctx, suite.addrs[0], "xrp", c("jpyx", 10000000))
	})
}
