...
func (suite *DrawTestSuite) SetupTest() {
	...
	suite.ctx = ctx
	suite.addrs = addrs
	err := suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 400000000), c("jpyx", 10000000), "xrp-a")
	suite.NoError(err)
}

func (suite *DrawTestSuite) TestAddRepayPrincipal() {

	err := suite.keeper.AddPrincipal(suite.ctx, suite.addrs[0], "xrp-a", c("jpyx", 10000000))
	suite.NoError(err)
	...
	suite.Equal(ts[0], t)
	tp := suite.keeper.GetTotalPrincipal(suite.ctx, "xrp-a", "jpyx")
	...
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[1], "xrp-a", c("jpyx", 10000000))
	...
	suite.Require().True(errors.Is(err, types.ErrInvalidDebtRequest))
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[0], "xrp-a", c("jpyx", 311000000))
	suite.Require().True(errors.Is(err, types.ErrInvalidCollateralRatio))

	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp-a", c("jpyx", 10000000))
	suite.NoError(err)
	...
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp-a", c("jpyx", 9000000))
...
func (suite *DrawTestSuite) TestRepayPrincipalOverpay() {
	err := suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[0], "xrp-a", c("jpyx", 20000000))
	suite.NoError(err)
	ak := suite.app.GetAccountKeeper()
	sk := suite.app.GetBankKeeper()
	acc := ak.GetAccount(suite.ctx, suite.addrs[0])
	suite.Equal(i(10000000000), sk.GetBalance(suite.ctx, acc.GetAddress(), "jpyx").Amount)
...
func (suite *DrawTestSuite) TestAddRepayPrincipalFees() {
	err := suite.keeper.AddCdp(suite.ctx, suite.addrs[2], c("xrp", 1000000000000), c("jpyx", 100000000000), "xrp-a")
	suite.NoError(err)
	...
	suite.NoError(err)
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[2], "xrp-a", c("jpyx", 10000000))
	suite.NoError(err)
	t, _ := suite.keeper.GetCdp(suite.ctx, "xrp-a", uint64(2))
	suite.Equal(c("jpyx", 92827), t.AccumulatedFees)
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[2], "xrp-a", c("jpyx", 100))
	suite.NoError(err)
	t, _ = suite.keeper.GetCdp(suite.ctx, "xrp-a", uint64(2))
	suite.Equal(c("jpyx", 92727), t.AccumulatedFees)
	err = suite.keeper.RepayPrincipal(suite.ctx, suite.addrs[2], "xrp-a", c("jpyx", 100010092727))
	suite.NoError(err)
	_, f := suite.keeper.GetCdp(suite.ctx, "xrp-a", uint64(2))
	suite.False(f)

	err = suite.keeper.AddCdp(suite.ctx, suite.addrs[2], c("xrp", 1000000000000), c("jpyx", 100000000), "xrp-a")
	suite.NoError(err)
	...
	err = suite.keeper.SynchronizeInterestForRiskyCdps(suite.ctx, sdk.Int{}, sdk.MaxSortableDec, "xrp-a")
	suite.NoError(err)
	err = suite.keeper.AddPrincipal(suite.ctx, suite.addrs[2], "xrp-a", c("jpyx", 100000000))
	suite.NoError(err)
...
func (suite *DrawTestSuite) TestPricefeedFailure() {
	...
	err := suite.keeper.AddPrincipal(ctx, suite.addrs[0], "xrp-a", c("jpyx", 10000000))
	suite.Error(err)
	err = suite.keeper.RepayPrincipal(ctx, suite.addrs[0], "xrp-a", c("jpyx", 10000000))
	suite.NoError(err)
}
...
func (suite *DrawTestSuite) TestModuleAccountFailure() {
	suite.Panics(func() {
		...
		suite.keeper.RepayPrincipal(ctx, suite.addrs[0], "xrp-a", c("jpyx", 10000000))
	})
}
