...
func (suite *InterestTestSuite) createCdps() {
	...
	err := suite.keeper.AddCdp(suite.ctx, addrs[0], c("xrp", 200000000), c("jpyx", 24000000), "xrp-a")
	...
	err = suite.keeper.AddCdp(suite.ctx, addrs[1], c("xrp", 200000000), c("jpyx", 10000000), "xrp-a")
	suite.NoError(err) // check that no error was thrown
...
func (suite *InterestTestSuite) TestSynchronizeInterest() {
	...
	testCases := []test{
		{
			...
			args{
				...
				initialPrincipal:        c("jpyx", 100000000000),
				timeElapsed:             oneYearInSeconds,
				expectedFees:            c("jpyx", 5000000000),
		...
		},
		{
			...
			args{
				ctype:                   "bjpy-a",
				initialTime:             time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC),
				initialCollateral:       c("bjpy", 10000000000000),
				initialPrincipal:        c("jpyx", 10000000000),
				timeElapsed:             7,
				expectedFees:            c("jpyx", 0),
				expectedFeesUpdatedTime: time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC).Add(time.Duration(int(time.Second) * 7)),
			},
		},
		{
			...
			args{
				...
				initialPrincipal:        c("jpyx", 10000000),
				timeElapsed:             7,
				expectedFees:            c("jpyx", 0),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			pk.SetPrice(suite.ctx, sdk.AccAddress{}, "bnb:jpy", d("17.25"), tc.args.expectedFeesUpdatedTime.Add(time.Second))
			pk.SetPrice(suite.ctx, sdk.AccAddress{}, "bjpy:jpy", d("1"), tc.args.expectedFeesUpdatedTime.Add(time.Second))
...
func (suite *InterestTestSuite) TestMultipleCDPInterest() {
	...
	testCases := []test{
		{
			...
			args{
				...
				initialCDPPrincipal:          c("jpyx", 500000000),
				numberOfCdps:                 100,
				expectedFeesPerCDP:           c("jpyx", 5),
				expectedTotalPrincipalPerCDP: c("jpyx", 500000005),
				expectedFeesUpdatedTime:      time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC).Add(time.Duration(int(time.Second) * 7)),
		...
		},
		{
			...
			args{
				...
				initialCDPPrincipal:          c("jpyx", 500000000),
				numberOfCdps:                 100,
				expectedFeesPerCDP:           c("jpyx", 541),
				expectedTotalPrincipalPerCDP: c("jpyx", 500000541),
		...
		},
		{
			...
			args{
				...
				initialCDPPrincipal:          c("jpyx", 500000000),
				numberOfCdps:                 100,
				expectedFeesPerCDP:           c("jpyx", 54152),
				expectedTotalPrincipalPerCDP: c("jpyx", 500054152),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			jpyxSupply := supplyTotal.AmountOf(types.DefaultStableDenom)
			...
			suite.Require().Equal(tc.args.expectedStableBalance, jpyxSupply)
...
func (suite *InterestTestSuite) TestCalculateCDPInterest() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			// setup pricefeed
			pk := suite.app.GetPriceFeedKeeper()
			pk.SetPrice(suite.ctx, sdk.AccAddress{}, "bnb:jpy", d("17.25"), tc.args.initialTime.Add(time.Duration(int(time.Second)*tc.args.timeElapsed)))
...
func (suite *InterestTestSuite) TestSyncInterestForRiskyCDPs() {
	...
	testCases := []test{

		{
			...
			args{
				...
				minPrincipal:       c("jpyx", 100000000),
				principalIncrement: c("jpyx", 10000000),
				initialTime:        time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC),
				timeElapsed:        oneYearInSeconds,
				expectedCDPs:       10,
			},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			// setup pricefeed
			pk := suite.app.GetPriceFeedKeeper()
			pk.SetPrice(suite.ctx, sdk.AccAddress{}, "bnb:jpy", d("20.0"), tc.args.initialTime.Add(time.Duration(int(time.Second)*tc.args.timeElapsed)))

			// setup cdp state
			suite.keeper.SetPreviousAccrualTime(suite.ctx, tc.args.ctype, suite.ctx.BlockTime())
			suite.keeper.SetInterestFactor(suite.ctx, tc.args.ctype, sdk.OneDec())
			for j, addr := range addrs {
				initialPrincipal := tc.args.minPrincipal.Add(c("jpyx", int64(j)*tc.args.principalIncrement.Amount.Int64()))
