...
func (suite *CdpTestSuite) TestAddCdp() {
	...
	suite.Equal(cs(c("jpyx", 10000000), c("xrp", 100000000), c("btc", 500000000)), acc.GetCoins())
	...
	acc = ak.GetAccount(suite.ctx, addrs[0])
	suite.Equal(cs(c("jpyx", 110000000), c("xrp", 100000000)), acc.GetCoins())
...
func (suite *CdpTestSuite) TestGetSetCdp() {
	...
	cdp := types.NewCDP(types.DefaultCdpStartingID, addrs[0], c("xrp", 1), "xrp-a", c("jpyx", 1), tmtime.Canonical(time.Now()), sdk.OneDec())
...
func (suite *CdpTestSuite) TestCalculateCollateralToDebtRatio() {
	_, addrs := app.GeneratePrivKeyAddressPairs(1)
	cdp := types.NewCDP(types.DefaultCdpStartingID, addrs[0], c("xrp", 3), "xrp-a", c("jpyx", 1), tmtime.Canonical(time.Now()), sdk.OneDec())
	...
	cdp = types.NewCDP(types.DefaultCdpStartingID, addrs[0], c("xrp", 1), "xrp-a", c("jpyx", 2), tmtime.Canonical(time.Now()), sdk.OneDec())
	cr = suite.keeper.CalculateCollateralToDebtRatio(suite.ctx, cdp.Collateral, cdp.Type, cdp.Principal)
