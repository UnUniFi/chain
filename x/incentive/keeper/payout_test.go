...
func (suite *KeeperTestSuite) TestPayoutJPYXMintingClaim() {
	...
	testCases := []test{
		{
			...
			args{
				...
				expectedPeriods:          vesting.Periods{vesting.Period{Length: 32918400, Amount: cs(c("ujsmn", 10571385600))}},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err = suite.keeper.ClaimJPYXMintingReward(suite.ctx, suite.addrs[0], tc.args.multiplier)
...
func (suite *KeeperTestSuite) TestPayoutHardLiquidityProviderClaim() {
	...
	testCases := []test{
		...
		},
		{
			...
			args{
				...
				expectedPeriods:          vesting.Periods{vesting.Period{Length: 32918400, Amount: cs(c("hard", 21142771200), c("ujsmn", 21142771200))}},
				isPeriodicVestingAccount: true,
			},
			errArgs{
				expectPass: true,
		...
		},
		{
			...
			args{
				...
				expectedPeriods:          vesting.Periods{vesting.Period{Length: 32140800, Amount: cs(c("hard", 211427712000), c("ujsmn", 211427712000))}},
				isPeriodicVestingAccount: true,
			},
			errArgs{
				expectPass: true,
		...
		},
		{
			...
			args{
				...
				expectedPeriods:          vesting.Periods{vesting.Period{Length: 32918400, Amount: cs(c("hard", 21142771200), c("ujsmn", 38399961600))}},
				isPeriodicVestingAccount: true,
			},
			errArgs{
				expectPass: true,
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			// setup jsmndist state
			sk := suite.app.GetSupplyKeeper()
			err := sk.MintCoins(suite.ctx, jsmndist.ModuleName, cs(c("hard", 1000000000000000000), c("ujsmn", 1000000000000000000)))
			suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestSendCoinsToPeriodicVestingAccount() {
	...
	tests := testCases{
		{
			...
			args: args{
				accArgs: accountArgs{
					periods: vesting.Periods{
						vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
						vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
						vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
						vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))}},
				...
				period:              vesting.Period{Length: 2, Amount: cs(c("ujsmn", 6))},
				ctxTime:             time.Unix(101, 0),
				mintModAccountCoins: true,
				expectedPeriods: vesting.Periods{
					vesting.Period{Length: 3, Amount: cs(c("ujsmn", 6))},
					vesting.Period{Length: 2, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))}},
		...
		},
		{
			...
			args: args{
				...
				period:              vesting.Period{Length: 7, Amount: cs(c("ujsmn", 6))},
				ctxTime:             time.Unix(80, 0),
				mintModAccountCoins: true,
				expectedPeriods: vesting.Periods{
					vesting.Period{Length: 7, Amount: cs(c("ujsmn", 6))},
					vesting.Period{Length: 18, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))}},
		...
		},
		{
			...
			args: args{
				...
				expectedPeriods: vesting.Periods{
					...
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 12, Amount: cs(c("ujsmn", 6))}},
				expectedStartTime: 100,
				expectedEndTime:   132,
			},
			errArgs: errArgs{
		...
		},
		{
			...
			args: args{
				...
				period:              vesting.Period{Length: 5, Amount: cs(c("ujsmn", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: vesting.Periods{
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 11))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))}},
				expectedStartTime: 100,
				expectedEndTime:   120,
			},
		...
		},
		{
			...
			args: args{
				...
				period:              vesting.Period{Length: 50, Amount: cs(c("ujsmn", 6))},
				ctxTime:             time.Unix(110, 0),
				mintModAccountCoins: true,
				expectedPeriods: vesting.Periods{
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					...
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 5, Amount: cs(c("ujsmn", 5))},
					vesting.Period{Length: 40, Amount: cs(c("ujsmn", 6))}},
				expectedStartTime: 100,
				expectedEndTime:   160,
			},
			errArgs: errArgs{
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			...
			// setup store state with account and jsmndist module account
			...
			if tc.args.mintModAccountCoins {
				sk := suite.app.GetSupplyKeeper()
				err = sk.MintCoins(suite.ctx, jsmndist.ModuleName, tc.args.period.Amount)
				suite.Require().NoError(err)
			}

			err = suite.keeper.SendTimeLockedCoinsToPeriodicVestingAccount(suite.ctx, jsmndist.ModuleName, pva.Address, tc.args.period.Amount, tc.args.period.Length)
...
func (suite *KeeperTestSuite) TestSendCoinsToBaseAccount() {
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, jsmndist.ModuleName, suite.addrs[1], cs(c("ujsmn", 100)), 5)
	suite.Require().NoError(err)
	acc := suite.getAccount(suite.addrs[1])
	vacc, ok := acc.(*vesting.PeriodicVestingAccount)
	suite.True(ok)
	expectedPeriods := vesting.Periods{
		vesting.Period{Length: int64(5), Amount: cs(c("ujsmn", 100))},
	}
	suite.Equal(expectedPeriods, vacc.VestingPeriods)
	suite.Equal(cs(c("ujsmn", 100)), vacc.OriginalVesting)
	suite.Equal(cs(c("ujsmn", 500)), vacc.Coins)
...
func (suite *KeeperTestSuite) TestSendCoinsToInvalidAccount() {
	suite.SetupWithAccountState()
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, jsmndist.ModuleName, suite.addrs[2], cs(c("ujsmn", 100)), 5)
	suite.Require().True(errors.Is(err, types.ErrInvalidAccountType))
	macc := suite.getModuleAccount(cdptypes.ModuleName)
	err = suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, jsmndist.ModuleName, macc.GetAddress(), cs(c("ujsmn", 100)), 5)
	suite.Require().True(errors.Is(err, types.ErrInvalidAccountType))
}
...
func (suite *KeeperTestSuite) SetupWithAccountState() {
	...
	macc := supplyKeeper.GetModuleAccount(ctx, jsmndist.ModuleName)
	err := supplyKeeper.MintCoins(ctx, macc.GetName(), cs(c("ujsmn", 600)))
	suite.Require().NoError(err)
	...
	periods := vesting.Periods{
		vesting.Period{Length: int64(1), Amount: cs(c("ujsmn", 100))},
		vesting.Period{Length: int64(2), Amount: cs(c("ujsmn", 100))},
		vesting.Period{Length: int64(8), Amount: cs(c("ujsmn", 100))},
		vesting.Period{Length: int64(5), Amount: cs(c("ujsmn", 100))},
	}
	bva, err2 := vesting.NewBaseVestingAccount(bacc, cs(c("ujsmn", 400)), ctx.BlockTime().Unix()+16)
