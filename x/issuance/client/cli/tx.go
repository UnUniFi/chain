...
func getCmdIssueTokens(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s issue 20000000jpytoken kava15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw
...
func getCmdRedeemTokens(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s redeem 20000000jpytoken
		`, version.ClientName, types.ModuleName),
...
func getCmdBlockAddress(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s block kava15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw jpytoken
		`, version.ClientName, types.ModuleName),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			inBuf := bufio.NewReader(cmd.InOrStdin())
...
func getCmdUnblockAddress(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s unblock kava15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw jpytoken
		`, version.ClientName, types.ModuleName),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			inBuf := bufio.NewReader(cmd.InOrStdin())
...
func getCmdPauseAsset(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s tx %s pause jpytoken true
		`, version.ClientName, types.ModuleName),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			inBuf := bufio.NewReader(cmd.InOrStdin())
