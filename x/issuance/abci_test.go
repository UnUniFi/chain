...
func (suite *ABCITestSuite) TestRateLimitingTimePassage() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			args{
				assets: issuance.Assets{
					issuance.NewAsset(suite.addrs[0], "jpytoken", []sdk.AccAddress{suite.addrs[1]}, false, true, issuance.NewRateLimit(true, sdk.NewInt(10000000000), time.Hour*24)),
				},
				supplies: issuance.AssetSupplies{
					issuance.NewAssetSupply(sdk.NewCoin("jpytoken", sdk.ZeroInt()), time.Hour),
				},
				blockTimes:     []time.Duration{time.Hour},
				expectedSupply: issuance.NewAssetSupply(sdk.NewCoin("jpytoken", sdk.ZeroInt()), time.Hour*2),
			},
		},
		{
			...
			args{
				...
				expectedSupply: issuance.NewAssetSupply(sdk.NewCoin("jpytoken", sdk.ZeroInt()), time.Duration(0)),
			},
		},
	}
