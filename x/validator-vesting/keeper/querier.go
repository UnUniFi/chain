...
func NewQuerier(keeper Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err error) {
		...
		case types.QueryCirculatingSupplyJPYX:
			return getCirculatingSupplyJPYX(ctx, req, keeper)
		case types.QueryTotalSupplyHARD:
			return getTotalSupplyHARD(ctx, req, keeper)
		case types.QueryTotalSupplyJPYX:
			return getCirculatingSupplyJPYX(ctx, req, keeper) // Intentional - JPYX total supply is the circulating supply
		default:
...
func queryGetTotalSupply(ctx sdk.Context, req abci.RequestQuery, keeper Keeper) ([]byte, error) {
	totalSupply := keeper.supplyKeeper.GetSupply(ctx).GetTotal().AmountOf("ujsmn")
...
func getCirculatingSupplyJPYX(ctx sdk.Context, req abci.RequestQuery, keeper Keeper) ([]byte, error) {
	totalSupply := keeper.supplyKeeper.GetSupply(ctx).GetTotal().AmountOf("jpyx")
	supplyInt := sdk.NewDecFromInt(totalSupply).Mul(sdk.MustNewDecFromStr("0.000001")).TruncateInt64()
	bz, err := types.ModuleCdc.MarshalJSON(supplyInt)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONUnmarshal, err.Error())
